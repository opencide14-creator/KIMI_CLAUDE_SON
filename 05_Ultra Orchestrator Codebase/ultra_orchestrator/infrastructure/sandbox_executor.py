...
class SandboxExecutor:
    ...
    def __init__(
        ...
        self.blocked_patterns: List[Tuple[str, str]] = (
            blocked_patterns if blocked_patterns is not None else BLOCKED_PATTERNS
        )
...
def scan_safety_sync(
    ...
    code : str
        Python source code to scan.
    blocked_patterns : list of (regex, description), optional
        Override the default :data:`BLOCKED_PATTERNS`.
    ...
    _bp = blocked_patterns if blocked_patterns is not None else BLOCKED_PATTERNS
    for pattern, description in _bp:
        if re.search(pattern, code, re.IGNORECASE):
            blocked_found.append(description)
