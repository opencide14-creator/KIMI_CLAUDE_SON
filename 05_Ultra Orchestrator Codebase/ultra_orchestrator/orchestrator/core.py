...
class OrchestratorCore:
    ...
    async def update_settings(self, settings: dict) -> dict:
        ...
        # Propagate sandbox blocked patterns
        sp = settings.get("sandbox_patterns")
        if sp is not None and hasattr(self, "sandbox_executor"):
            ...
            for entry in sp:
                if isinstance(entry, dict):
                    ...
                    if pat:
                        parsed.append((pat, desc))
                elif isinstance(entry, (list, tuple)) and len(entry) >= 2:
                    parsed.append((str(entry[0]), str(entry[1])))
            if parsed:
                self.sandbox_executor.blocked_patterns = parsed

        # Propagate quality gate banned patterns
        bp = settings.get("banned_patterns")
        if bp is not None and hasattr(self, "quality_gate"):
            if isinstance(bp, dict):
                import re as _re

                self.quality_gate.banned_patterns = {
                    _re.compile(pat, _re.IGNORECASE) for pat in bp.keys()
                }

        # Propagate hard_limits scalars if present
        hl = settings.get("hard_limits", {})
        if "max_concurrent" in hl:
            self._max_concurrent = max(1, int(hl["max_concurrent"]))
        if "api_max_retries" in hl:
            self._max_retries = max(1, int(hl["api_max_retries"]))
            if hasattr(self, "state_machine"):
                self.state_machine.max_retries = self._max_retries
            if hasattr(self, "retry_engine"):
                self.retry_engine.max_retries = self._max_retries
