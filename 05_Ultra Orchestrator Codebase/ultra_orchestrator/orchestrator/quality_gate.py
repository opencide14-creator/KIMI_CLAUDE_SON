...
class QualityGate:
    ...
    def __init__(
        ...
        banned_patterns: dict[str, str] | None = None,
        duplicate_similarity_threshold: float | None = None,
        max_unused_import_ratio: float | None = None,
    ) -> None:
        ...
        # Compile banned patterns into regex objects (override via param)
        _bp = banned_patterns if banned_patterns is not None else BANNED_PATTERNS
        self.banned_patterns: set[re.Pattern] = {
            re.compile(pattern, re.IGNORECASE) for pattern in _bp
        }
        ...
        self.max_unused_import_ratio: float = (
            max_unused_import_ratio
            if max_unused_import_ratio is not None
            else MAX_UNUSED_IMPORT_RATIO
        )
    ...
    async def _layer_4_semantic_deduplication(self, output: str, subtask: dict) -> tuple[bool, str]:
        ...
        for entry in self.approved_outputs:
            ...
            if matcher.ratio() > self.duplicate_similarity_threshold:
                prev_id = entry.get("subtask_id", "unknown")
                return False, f"Near-duplicate of {prev_id} (similarity > {self.duplicate_similarity_threshold})"
    ...
    async def _check_unused_imports(self, tree: ast.AST) -> tuple[bool, str]:
        ...
        if ratio > self.max_unused_import_ratio:
            unused_list = [name for name, count in imported_names.items() if count == 0]
            return (False,
                    f"Unused imports exceed limit ({unused_count}/{total_imports}): {', '.join(unused_list[:5])}")
