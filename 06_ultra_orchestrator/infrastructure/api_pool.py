...
_BACKOFF_SCHEDULE = {1: 60, 2: 300}  # 1st→60s, 2nd→300s, 3rd+→900s
_BACKOFF_DEFAULT = 900

# Circuit breaker thresholds
_CIRCUIT_OPEN_AFTER_429S = 3
...
class KimiAPIPool:
    ...
    def _load_keys(self) -> list[tuple[str, str]]:
        ...
        Variables: ``KIMI_KEY_1``, ``KIMI_KEY_2``, ``KIMI_KEY_3``, ``KIMI_KEY_4``
        Missing variables are skipped with a warning log.
        """
        ...
        for i in range(1, _NUM_KEYS + 1):
            ...
            if not value:
                logger.warning(
                    "Environment variable %s not set — skipping", env_name
                )
                continue
            key_id = f"KEY_{i}"
    ...
    async def get_optimal_key(self, estimated_tokens: int = 1_000) -> str | None:
        ...
        From the passing candidates the one with the highest
        ``capacity_ratio`` is chosen.
        ...
        for key_id, bucket in self.buckets.items():
            async with bucket.lock:
                ...
                if bucket.can_handle(estimated_tokens):
                    candidates.append((key_id, bucket.get_capacity_ratio()))

        if not candidates:
            return None

        # Pick the candidate with the highest capacity ratio
        candidates.sort(key=lambda x: x[1], reverse=True)
        chosen = candidates[0][0]
        logger.debug(
            "Selected %s (capacity=%.2f%%) for ~%d tokens",
            chosen,
            candidates[0][1] * 100,
            estimated_tokens,
        )
    ...
    async def send_request(
        ...
        body_json = self._build_body(messages, temperature, max_tokens)
